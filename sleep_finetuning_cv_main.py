...
from src.trainers.finetuner import Finetuner
...
def get_roformer_classifier(args):
...
def freeze_lstm(model):
    i = 0
    for name, param in model.named_parameters():
        if name.split(".")[0] == "lstm":
            param.requires_grad = False
            i += 1
    print("Weights being frozen: %d" % i)

def get_sleepnet(args):
    model = sleepnet(
        pretrained=True,
        my_device="cpu",
        num_classes=5,
    ...
    if args.finetune_train == "head_only":
        freeze_feature_extractor(model)
        freeze_lstm(model)
    elif args.finetune_train == "lstm_head":
        freeze_feature_extractor(model)
    elif args.finetune_train == "full":
        pass
    else:
        raise ValueError("finetune_train must be one of 'head_only' 'lstm_head' or 'full'")    
    return model
...
def finetune(args, device, distributed, fold = None):
    ...
    if args.model_type == 'mae':
        model = get_roformer_classifier(args)
    elif args.model_type == 'harnet':
        model = get_harnet(args)
    elif args.model_type == 'sleepnet':
        model = get_sleepnet(args)
    ...
    trainer = Finetuner(
        ...
        label_map = {
            int_label: str_label 
            for str_label, int_label 
            in train_loader.dataset.label_map.items()
            if int_label != train_loader.dataset.ignore_index
        },
        ignore_index = train_loader.dataset.ignore_index,
    )
