...
def main(args):
    ...
    # subjects for whom calibration fails without setting calib_cube to -1
    # ['HUA11', 'HUA15', 'HUA50', 'HUA26', 'HUA52']
    # Make a list of subjects ids for subjects with actigraphy data
    ...
    for act_h5 in act_files:
        ...
        if len(unique_fs) > 1:
            print(f'Found multiple fs after dropping NaNs: {unique_fs} for {act_h5}, skipping. \n')
        ...
        annotation_paths = {
            tag: glob.glob(f'{subject_dir}/**/{annot_file}.txt', recursive=True)[0]
            for annot_file, tag
            in args.annotation_file_tags.items()
        }

        annot_df = io_utils.get_annotations(acc_df_preproc.index, annotation_paths, study_start)

        print(annot_df['apnea'].unique())
        ...
        print(annot_df['hypnogram'].unique())
