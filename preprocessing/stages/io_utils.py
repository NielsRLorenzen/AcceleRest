...
def get_annotations(
    ...
    for tag, annot_path in annotation_paths.items():                                
        ...
        if tag == 'hypnogram':
            ...
            annot_df.loc[annot_df[tag] == 'Artefact', tag] = 'missing'
            annot_df.loc[annot_df[tag] == 'Rem', tag] = 'REM'
...
def read_labels(label_file_path:str, study_start:pd.Timestamp) -> pd.DataFrame:
    ...
    try:
        ...
        print(f'Error {e}, occured for file {label_file_path}')
        signal_type = 'event' if signal_type == 'Impuls' else 'continuous'
        return pd.DataFrame(), signal_type
